...
public class FPSController : MonoBehaviour {
	...
	void Update () {
		...
		// Si FullAuto esta desactivado y pulsamos Fire 1, llamamos al disparo
		if (!fullAuto && Input.GetButtonDown("Fire1")) {
			BroadcastMessage ("CallShoot");
		}

		// Si FullAuto esta activado y dejamos pulsado Fire 1, llamamos a disparo tantas veces como se pueda
		if (fullAuto && Input.GetButton("Fire1")) {
			BroadcastMessage ("CallShoot");
		}
		// Si pulsamos B y FullAuto esta desactivado, lo activamos
		if (!fullAuto && Input.GetKeyDown(KeyCode.B)) {
			fullAuto = true;
			Debug.Log ("FullAuto ON");
		// Si pulsamos B y FullAuto esta activado, lo desactivamos 
		} else if (fullAuto && Input.GetKeyDown(KeyCode.B)) {
			fullAuto = false;
			Debug.Log ("FullAuto OFF");
		}
		// Si pulsamos Reload, llamamos a la recarga
